...
INDICADORES <- list(
  ...
  IND("deficit_cuali", "Déficit cualitativo de vivienda", "03_Ordenamiento",
      ...
      "fila", -1, "pct", "Ordenamiento"),
  # "fila": la hoja ya publica el agregado provincial. Antes leían el
  # promedio simple de la columna "(municipal)", que sobrepondera a los
  # municipios pequeños. Hallazgo de Pablo (F-2-049), adoptado también aquí.
  ...
  IND("mortalidad_infantil", "Mortalidad infantil (x 1.000 n.v.)", "09_Salud",
      ...
      "Salud"),
  IND("bajo_peso", "Bajo peso al nacer", "09_Salud", "salud.xlsx", "bajo_peso_2024",
      "Nacidos con bajo peso al nacer, 2024 (%)", "fila", -1, "pct", "Salud"),
...
construir_panel <- function() {
  ...
  # El comparativo ES la comparación de las once: uno parcial no es una
  # versión reducida, es otro documento que dice lo que no es (a diferencia
  # del anexo metodológico, que sí puede describir una cobertura parcial —
  # ver corrección 14). run_all.R no incluye esta etapa y declara el estado
  # parcial como normal, así que nada garantizaba que las once hubieran
  # corrido antes de llegar aquí. Una provincia AUSENTE (ningún indicador con
  # dato) aborta; una provincia con huecos (algunos indicadores sin dato) solo
  # avisa, porque eso es un caso legítimo que el ranking ya sabe manejar.
  # Hallazgo de Pablo (F-2-054), adoptado también aquí.
...
cobertura_sistema <- function(panel) {
  ...
  # El valor departamental es el mismo se entre por la provincia que se
  # entre, pero solo lo publica la hoja de una provincia que exista. Buscarlo
  # únicamente en provincia(1) hacía que un fallo en esa sola carpeta vaciara
  # la cobertura del sistema entera. .valor_departamental() ya resuelve esto
  # recorriendo las once hasta encontrar la hoja. Hallazgo de Pablo
  # (F-2-054), adoptado también aquí.
