...
figuras_demografia <- function(wb) {
  ...
  wb <- .try("fig5 urbana-rural", wb, { d <- leer_municipios(wb, "poblacion")
    d5 <- data.frame(Municipio = pick(d, "municipio"),
                     `% Urbana` = pick(d, "porcentaje de poblacion urbana"),
                     `% Rural`  = pick(d, "porcentaje de poblacion rural"), check.names = FALSE)
    enc_hbar(wb, "poblacion", "O2", "_fig_urbanarural", d5,
             cat_col = "Municipio", value_cols = c("% Urbana", "% Rural"),
             colors = c(tema$navy, tema$blue),
             titulo = "Composición urbana-rural por municipio (%)",
             grouping = "percentStacked", label_series = 1, num_fmt = "0.0%",
             label_color = "FFFFFF", legend = TRUE) })

  # Fig 6 — Densidad poblacional por municipio (barra simple)
  wb <- .try("fig6 densidad poblacional", wb, { d <- leer_municipios(wb, "poblacion")
    d6 <- data.frame(Municipio = pick(d, "municipio"),
                     Densidad = pick(d, "densidad poblacional"), check.names = FALSE)
    enc_hbar(wb, "poblacion", "O24", "_fig_densidad", d6,
             cat_col = "Municipio", value_cols = "Densidad", colors = tema$navy,
  ...
  wb <- .try("fig9 indice envejecimiento", wb, { e <- leer_municipios(wb, "natalidad_mortalidad")
    d9 <- data.frame(Municipio = pick(e, "municipio"),
                     `Índice` = pick(e, "indice de envejecimiento"), check.names = FALSE)
    enc_hbar(wb, "natalidad_mortalidad", "I2", "_fig_envejecimiento", d9,
             cat_col = "Municipio", value_cols = "Índice", colors = tema$navy,
             titulo = "Índice de envejecimiento por municipio - 2024",
             grouping = "standard", num_fmt = "0.0", legend = FALSE) })

  # Fig 7 — Pirámide poblacional (género × grupos de edad), nivel provincia
  wb <- .try("fig7 piramide poblacional", wb, { ee <- openxlsx2::wb_to_df(wb, sheet = "estructura_edad")
    ...
    enc_piramide(wb, "estructura_edad", "AI2", "_fig_piramide", bandas, h, m,
...
tablas_demografia <- function(wb) {
  ...
  wb <- .try("tabla poblacion", wb, estilo_tabla(wb, "poblacion", formats = list(
  ...
  wb <- .try("tabla natalidad_mortalidad", wb, estilo_tabla(wb, "natalidad_mortalidad", formats = list(
    "tasa de natalidad" = dec2, "tasa de mortalidad" = dec2,
    "indice de envejecimiento" = dec2, "codigo dane" = "0")))

  # Tabla — Tasa neta de migración
  wb <- .try("tabla migracion", wb, estilo_tabla(wb, "migracion", formats = list(
